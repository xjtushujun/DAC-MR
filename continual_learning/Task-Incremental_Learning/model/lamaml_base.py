...
class BaseNet(torch.nn.Module):
    ...
    def getBatch(self, x, x_s, y, t, batch_size=None):
        """
        ...
        for j in range(len(myi)):
            bxs.append(mxi[j])
            bxs_s.append(mxi_s[j])
            bys.append(myi[j])
            bts.append(mti[j])

        bxs = Variable(torch.from_numpy(np.array(bxs))).float() 
        bxs_s = Variable(torch.from_numpy(np.array(bxs_s))).float() 
