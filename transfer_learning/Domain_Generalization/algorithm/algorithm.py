...
class ERM(nn.Module):
    ...
    def learn(self, images, labels, images_s=None, group_ids=None, epoch=-1):
        ...
        if epoch >20:
            logits_s = self.predict(images_s)     
            pred_u_w = F.softmax(logits, dim=1).detach().data
            pred_u_s = F.log_softmax(logits_s, dim=1)
            l_g_meta = F.kl_div(pred_u_s, pred_u_w, reduction='batchmean')
            loss = self.loss_fn(logits, labels) + l_g_meta
        else:         
...
class ARM_LL(ERM):
    ...
    def predict(self, x, labels=None, x_s=None, epoch=-1, backprop_loss=False):
        ...
        for domain_id in range(n_domains):
            ...
            if epoch>20:
              ...
              with higher.innerloop_ctx(
                ...
                domain_logits_s = fnet(domain_x_s)
                ...
                if backprop_loss and labels is not None:
                    ...
                    pred_u_w = F.softmax(domain_logits, dim=1).detach().data
                    pred_u_s = F.log_softmax(domain_logits_s, dim=1)
                    l_g_meta = F.kl_div(pred_u_s, pred_u_w, reduction='batchmean')
                   
                    domain_loss = self.loss_fn(domain_logits, domain_labels) + l_g_meta
    ...
    def learn(self, x, labels, x_s=None,group_ids=None, epoch=-1):
        self.train()
        logits, loss = self.predict(x, labels, x_s, epoch, backprop_loss=True)
        self.optimizer.step()
        self.optimizer.zero_grad()
