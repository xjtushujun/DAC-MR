...
def run_epoch(algorithm, loader, train, progress_bar=True, epoch=0):
    ...
    if train:
        for (images, images_s), labels, group_ids in loader:
            # Put on GPU
            images = images.to(algorithm.device)
            images_s = images_s.to(algorithm.device)
            labels = labels.to(algorithm.device)
            # Forward
            logits, batch_stats = algorithm.learn(images, labels, images_s, group_ids, epoch)
...
def train(args):
    ...
    for epoch in trange(args.num_epochs):
        epoch_logits, epoch_labels, epoch_group_ids = run_epoch(algorithm, train_loader, train=True, progress_bar=args.progress_bar, epoch=epoch)
