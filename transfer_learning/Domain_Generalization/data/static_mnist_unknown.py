...
def rotate1(X, rotation, single_image=False):
    if single_image:
        return np.array(sp.ndimage.rotate(X, rotation, reshape=False, order=0))
    else:
        return np.array(
            [sp.ndimage.rotate(X[i], (rotation[i]+1) % 14, reshape=False, order=0)
             for i in range(X.shape[0])]
        )
    ...
    def __getitem__(self, index):
      if self.split == 'train':
        ...
        if self.apply_transform:
            ...
            img_w = rotate(img, group_value, single_image=True)
            img_s = rotate1(img, group_value, single_image=True)
            
        img_w = rescale(img_w) # =/ 256
        img_s = rescale(img_s) # =/ 256
        if self.binarize:
            img_w = np.random.binomial(1, img_w).astype(np.float32)
            img_s = np.random.binomial(1, img_s).astype(np.float32)
            
        img_w = torch.tensor(img_w, dtype=torch.float)
        img_s = torch.tensor(img_s, dtype=torch.float)
        
        # Put color channel first
        img_w = img_w.permute(-1, 0, 1)
        img_s = img_s.permute(-1, 0, 1)

        label = self.labels[index]
        label = torch.tensor(label, dtype=torch.long)


        return (img_w, img_s), label, group_id
