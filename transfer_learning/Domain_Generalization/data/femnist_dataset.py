...
class TransformFix(object):
    def __init__(self):
        ...
        self.strong = albumentations.Compose([
              albumentations.RandomSizedCrop([24, 24],width=28, height=28),
              albumentations.Blur(blur_limit=7, p=0.8),
    ...
    def __call__(self, x):
        ...
        strong = self.strong(**{'image': x})['image']
        return (weak), (strong)


class TransformFix1(object):
    def __init__(self):
        self.weak = albumentations.Compose([
              ToTensor()
              ])
...
class FEMNISTDataset(Dataset):
    ...
    def __init__(self, split, root_dir, args):
        ...
        if split == 'train':
            self.transform = TransformFix()
        else:
            self.transform = TransformFix1()  
    ...
    def __getitem__(self, index):
        x = self.transform( self._X[index] )
