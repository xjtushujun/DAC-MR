...
from fix_utils1 import ImageClassifier
from sam import SAM
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator,
    ...
    for i in range(args.iters_per_epoch):
        ...
        y, f, f1 = model(x)
        y_t_u, f_t_u, f_t_u1 = model(x_t_u)

        f_s, f_t = f.chunk(2, dim=0)
        y_s, y_t = y.chunk(2, dim=0)

        f_s1, f_t1 = f1.chunk(2, dim=0)
        ...
        if epoch > 10:
            pred_u_w = F.softmax(f_t1, dim=1).detach().data
            pred_u_s = F.log_softmax(f_t_u1, dim=1)
            kl_loss = torch.mean( -torch.sum((pred_u_w * pred_u_s), 1) )
            # print('target_train_pred_r_u:', target_train_pred_r_u[0], target_train_pred_r[0], F.softmax(target_train_pred_r, dim=1).detach().data[0], F.softmax(target_train_pred_r_u, dim=1).detach().data[0])

            # kl_loss = torch.mean( -torch.sum((target_train_pred_r_u - target_train_pred_r) ** 2, 1) )

            # print('kl_loss:', kl_loss, Lu)
        else:
            kl_loss = 0.
        ...
        if Lu != 0:
            loss = cls_loss + transfer_loss * args.trade_off + Lu * args.trade_off3 + (0.3) * kl_loss
        else: 
            loss = cls_loss + transfer_loss * args.trade_off + (0.3) * kl_loss
        ...
        if Lu != 0:
            loss1 = cls_loss + transfer_loss * args.trade_off + Lu * args.trade_off3 + (0.3) * kl_loss
        else: 
            loss1 = cls_loss + transfer_loss * args.trade_off + (0.3) * kl_loss
